/************************************************************************/
/* taxsolve_ma_1_2017.c - OpenTaxSolver for Mass Form 1 		*/
/* Copyright (C) 2017 							*/
/* 									*/
/* OTS Project Home Page and Updates:  					*/
/*		http://opentaxsolver.sourceforge.com/			*/
/* 									*/
/* Compile:   cc taxsolve_ma_1_2017.c -o taxsolve_ma_1_2017		*/
/* Run:       ./taxsolve_ma_1_2017  Mass1_2017.txt			*/
/* 									*/
...
/*  ...									*/
/* Updated 1-25-2018	Aston Roberts 					*/
/************************************************************************/
...
{ double taxrate=0.051;					/* NOT Updated for 2017. */
...
int main( int argc, char *argv[] )
{
 ...
 printf("Massachusetts Form-1 2017 - v%3.2f\n", thisversion);
 ...
 */
 ...
 GetLine( "L37", &L[37] );	/* Mass income tax withheld, Forms W-2, 1099 */
 ShowLineNonZero(37);

 GetLine( "L38", &L[38] );	/* Last year's overpayment you want applied to 2017 estimated tax */
 ShowLineNonZero(38);

 GetLine( "L39", &L[39] );	/* 2017 estimated tax payments */
 ShowLineNonZero(39);

 GetLine( "L40", &L[40] );	/* Payments made with extension */
 ShowLineNonZero(40);

 GetLine( "L41", &L[41] );	/* Payments w/original return. Use only if amending return. */
 ShowLineNonZero(41);

 GetLine( "L42a", &L42a );	/* Earned income credit (EIC) */
 if (L42a != 0.0) fprintf(outfile, " L42a = %6.2f  x 0.23 = .....  ", L42a );
 L[42] = L42a * 0.23;
 ShowLineNonZero(42);

 GetLine( "L43", &L[43] );	/* Senior Circuit Breaker Credit, sched CB */
 ShowLineNonZero(43);

 GetLine( "L44", &L[44] );	/* Refundable credits, Sched RF, line 4. */
 ShowLineNonZero(43);

 L[45] = Sum( L, 37, 44 );
 showline_wmsg(44,"total payments");

 GetLine( "L47", &L[47] );	/* Overpayment to be applied to next year's estimated tax */
