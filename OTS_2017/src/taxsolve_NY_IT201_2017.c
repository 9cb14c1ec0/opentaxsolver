...
double TaxRateFunction( double income, int status )
{
 ...
 switch (status)
  {
   case MARRIED_FILLING_JOINTLY: case WIDOW:					/* Updated for 2017. */
...
double NYcityTaxRateFunction( double income, int status )	/* From page 68. */
{
 ...
 if ((status==MARRIED_FILLING_JOINTLY) || (status==WIDOW))		/* Updated for 2017. */
  {
...
void worksheet1()	/*Tax Computation Worksheet 1 (pg 58) */		/* Updated for 2017. */
{ double ws[100];
  ws[1] = L[33];
  ws[2] = L[38];
  ws[3] = 0.0645 * ws[2];
...
{ /* Worksheets from pages 58-60. Come here when AGI L[33] > $106,950. */
 switch (status)								/* Updated for 2017. */
  {
 ...
 switch (status)	/* Determine the Std. Deduction. Pg. 20. */
  {
   case SINGLE: if (Dependent)   std_ded = 3100.0; 
		else 		 std_ded = 8000.0;			/* Updated for 2017. */
	break;
